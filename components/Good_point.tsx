...
const ContentSection = ({
  ...
  const textBlock = (
    ...
    >
      <h1 className="text-5xl md:text-7xl font-extrabold text-[#9A3434] uppercase leading-none tracking-tight">
        {title}
      </h1>
      <div className={`mt-4 md:mt-6 text-base md:text-lg text-[#9A3434]/80 ${expanded ? '' : 'line-clamp-1'}`}>{children}</div>
      <button
        onClick={() => setExpanded((v) => !v)}
        className="mt-2 inline-flex items-center gap-1 text-sm text-[#9A3434] underline md:no-underline md:border md:border-[#9A3434] md:px-3 md:py-1 md:rounded-full"
  ...
  return (
    <motion.section
      ...
      className="flex flex-col md:flex-row justify-center items-center w-full max-w-7xl mx-auto py-24 px-8"
    >
